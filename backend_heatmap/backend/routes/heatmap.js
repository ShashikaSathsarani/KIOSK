  ...
  try {
    ...
    if (dbResult.rows.length > 0) {
      ...
      for (let row of dbResult.rows) {
        ...
        // treat cache as expired if older than 1 minute
        if (diff > 1) {
          useCache = false;
          console.log(`Cache expired for building_id ${row.building_id} (age ${diff.toFixed(2)} min), fetching fresh data.`);
          break;
        }
      }
    } else {
    ...
    for (let building of buildings) {
      ...
      // Only insert/update DB if the building exists in our capacities map (prevents FK violations)
      if (capacityMap[building.building_id] !== undefined) {
        try {
          await pool.query(
          ...
          console.warn(`Skipping DB update for building_id=${building.building_id}:`, e.message);
        }
      } else {
        console.warn(`Unknown building_id ${building.building_id} — skipping DB update to avoid FK error.`);
      }
    }
