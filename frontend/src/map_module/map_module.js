...
// Re-use helper functions from the legacy utils implementation where needed
import { getRouteToNode, getRouteToBuilding, getGpsPosition } from "../utils/map_module.js";
...
function drawRoute(result) {
  ...
  console.log(result)
  // Guard: map or pane may not be initialized yet if this is called early
  if (!map) {
    console.warn('drawRoute called before map initialized. Skipping draw.');
    return;
  }

  // Ensure the pane exists (create if missing)
  let routePaneEl = (typeof map.getPane === 'function') ? map.getPane('routePane') : null;
  if (!routePaneEl) {
    try {
      map.createPane('routePane');
      map.getPane('routePane').style.zIndex = 650;
      routePaneEl = map.getPane('routePane');
    } catch (e) {
      console.warn('Could not create or access routePane:', e);
    }
  }

  if (routePaneEl && routePaneEl.querySelectorAll) {
    routePaneEl.querySelectorAll('path, circle, polygon').forEach(el => el.remove());
  }
